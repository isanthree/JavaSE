...
public class ObjectInputOutputStreamTest {
	...
	public void testObjectOutputStream() {
		...
		try {
			// 1.造文件、造流
			oos = new ObjectOutputStream(new FileOutputStream("dataDir\\object.dat"));
			// 2. 写操作
			oos.writeObject(new String("I Love China!"));
			oos.flush();  // 刷新操作

			oos.writeObject(new Person("hhh", 19));
			oos.flush();

			oos.writeObject(new Person("yyy", 19, 1001, new Account(5000)));
			oos.flush();
	...
	public void testObjectInputStream() {
		...
		try {
			ois = new ObjectInputStream(new FileInputStream("dataDir\\object.dat"));
