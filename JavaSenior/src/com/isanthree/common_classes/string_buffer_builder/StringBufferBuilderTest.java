package com.isanthree.common_classes.string_buffer_builder;
...
public class StringBufferBuilderTest {
	...
	public void test2() {
		...
		System.out.println("s1 原始值：" + s1);
		s1.append(1);
		s1.append('1');
		System.out.println("s1 尾插入 11：" + s1);
		s1.delete(2, 4);  // 删除区间 [2, 4)
		System.out.println("s1 删除区间 [2, 4)：" + s1);
		s1.replace(2, 4, "hello");  // [2, 4) 的字符替换成 hello
		System.out.println("s1 将区间 [2, 4) 字符替换为 hello：" + s1);
		s1.insert(2, false); // 索引 2 的位置插入 "false"
		System.out.println("s1 在索引 2 的位置插入 \"false\"：" + s1);
		s1.reverse();
		System.out.println("s1 字符串反转：" + s1);
		System.out.println("s1 字符串长度：" + s1.length());
		String s2 = s1.substring(1, 3);  // 取区间 [1, 3)
		System.out.println("s2 取区间 [1, 3)：" + s2);
	}
